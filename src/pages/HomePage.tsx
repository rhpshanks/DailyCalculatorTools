...
export default function HomePage() {
  ...
  const filteredTools = searchQuery 
    ? tools.filter(t => 
        ...
        t.benefit.toLowerCase().includes(searchQuery.toLowerCase())
      )
    : tools;
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-8">
            {searchQuery ? (
              ...
              <div>
                ...
                {filteredTools.length > 0 ? (
                  ...
                  <div className="text-center py-16 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                    <p className="text-lg text-slate-500 dark:text-slate-400">No tool found matching "{searchQuery}"</p>
                    <button
                      onClick={() => setSearchQuery('')}
                      className="mt-4 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm rounded-lg cursor-pointer"
                    >
                      Browse All Tools
                    </button>
                  </div>
              ...
              // Standard Homepage View: Featured Calculator + Details
              <div className="flex flex-col gap-8">
                ...
                {/* 2. Details Grid (Formula, Example, Common Values, FAQs, Related Tools) */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800/80 p-6 shadow-sm transition-colors md:col-span-1">
                    ...
                    <div className="mt-4 text-center">
                      <button onClick={() => setSearchQuery('Percentage')} className="text-xs font-bold text-blue-600 dark:text-blue-400 hover:underline inline-flex items-center gap-0.5 cursor-pointer">
                        <span>View more values</span>
                  ...
                  <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800/80 p-6 shadow-sm transition-colors md:col-span-2">
                    ...
                    <div className="mt-5 text-center">
                      <button onClick={() => setSearchQuery('Percentage')} className="text-xs font-bold text-blue-600 dark:text-blue-400 hover:underline inline-flex items-center gap-0.5 cursor-pointer">
                        <span>View all percentage tools</span>
                        <ChevronRight className="w-3.5 h-3.5" />
                      </button>
                    </div>
                  </div>
